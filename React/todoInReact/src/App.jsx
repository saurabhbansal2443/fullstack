...
const App = () => {
  ...
  return (
   ...
   <button className='border-2 border-orange-300 w-1/12 rounded-xl text-3xl hover:bg-slate-500 shadow-2xl' onClick={addTodo}> Add </button>
   </div>
   <div className=' flex justify-center h-3/5 items-center' >
   <ul className="w-96">
    {task.map((taskEle ,idx)=>{
      return <li key={idx}
      className="w-full border-b-4 border-neutral-400 border-opacity-900 py-4 dark:border-opacity-500 flex justify-between shadow-2xl rounded-xl px-3 my-2 text-3xl">
       {taskEle}
